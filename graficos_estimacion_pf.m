...
figure(2), subplot(2,1,1), plot([soc_counting(s_est.tpo_predic) soc_counting(s_est.tpo_predic)],[min(min(imp),min(min(squeeze(s_est_hist.part(:,:,1))))), max(max(imp),max(max(squeeze(s_est_hist.part(:,:,1)))))],'k--')
...
figure(2), subplot(2,1,1), set(gca,'children',flipud(get(gca,'children')))
...
%figure(2), subplot(2,1,2), stairs(soc_counting(1:s_est.tpo_predic),s_est_hist.ofcl);
...
%figure(2), legend('V_{data}','V_{filtered}','OFCL status','Location','West')
